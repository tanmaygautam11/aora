...
export const toggleBookmark = async (videoId) => {
  try {
    ...
    // Get the current bookmark value
    const currentBookmark = post.bookmark?.$id || null;
    
    // Update the bookmark value: Add `currentUserId` if not bookmarked, otherwise set to `null`
    const updatedFields = {
      bookmark: currentBookmark !== currentUserId ? currentUserId : null,
    };
    
    // Update the document with the new bookmark value
...
export const deleteVideo = async (videoId) => {
  try {
    // Deleting the video document from the database
